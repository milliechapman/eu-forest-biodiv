...
spp_range <- read_fst("data/formatted-data/features_data_spp_fortargets.fst") |>
  filter(zone == 25, amount >0) |> group_by(pu, feature, zone) |> summarise(amount = sum(amount))

rwr <- spp_range |>
  group_by(feature) |> mutate(total = sum(amount)) |>
  mutate(perc_range = amount/total) |>
  group_by(pu) |>
  summarise(rwr = sum(perc_range))

rwr$rwr <- rwr$rwr / max((rwr$rwr), na.rm = TRUE)
...
rwr_plot <- pu_in_EU |> mutate(id = pu) |> #plot_data |>
  left_join(PU_template) |>
  dplyr::select(pu, EU_id, geometry) |> left_join(rwr) |>
  st_as_sf()

rwr_plot <- fasterize(st_as_sf(rwr_plot), raster, field = "rwr")
plot(rwr_plot)
writeRaster(rwr_plot, "data/forest_rwr.tif")


richness <- spp_range |>
  group_by(pu) |>
  summarise(richness = sum(amount))

richness$richness <- richness$richness / max(richness$richness, na.rm = TRUE)

richness_plot <- pu_in_EU |> mutate(id = pu) |> #plot_data |>
  left_join(PU_template) |>
  dplyr::select(pu, EU_id, geometry) |> left_join(richness) |>
  st_as_sf()

richness_plot <- fasterize(st_as_sf(richness_plot), raster, field = "richness")

plot(richness_plot)

writeRaster(richness_plot, "data/forest_richness.tif")

targs_existing <- read_csv("data/formatted-data/targets_split.csv") |>
  mutate(feature = substr(feature, 1,7)) |>
  group_by(feature) |>
  summarise(target = sum(target)) |>   mutate(feature = as.numeric(feature)) |>
  left_join(MAES |> dplyr::select(maes_label, typeasso, taxon_id) |> rename(feature = taxon_id), multiple = "all") |>
  filter(maes_label == "WoodlandForest") |> arrange(-target)
